...
function sudokuReducer(state, action) {
    switch (action.type) {
        ...
        case ACTIONS.UPDATE_CELL: {
            ...
            return {
                ...state, board: newBoard, invalidCells, isComplete, isTimerRunning: !isComplete, // Stop timer when complete
            };
        }
        ...
        case ACTIONS.SET_GAME_COMPLETE:
            return {
                ...state, isComplete: true, isTimerRunning: false,
            };
...
export function SudokuProvider({children}) {
    ...
    const actions = {
        ...
        showHint: () => {
            ...
            if (hint) {
                dispatch({
                    type: ACTIONS.SHOW_HINT, payload: [hint.row, hint.col]
                });
            }
    ...
    return (<SudokuContext.Provider value={{...state, ...actions}}>
        {children}
