...
export default function SudokuBoard({
    ...
    const isGiven = (row, col) => Array.isArray(givenCells) && givenCells.some(([r, c]) => r === row && c === col);

    const isSelected = (row, col) => selectedCell?.[0] === row && selectedCell?.[1] === col;

    const isInvalid = (row, col) => invalidCells.some(([r, c]) => r === row && c === col);

    const isHint = (row, col) => hintCell?.[0] === row && hintCell?.[1] === col;
    ...
    return (<div className="board-wrap">
        <table className="board" role="grid" aria-label={`Sudoku ${size} by ${size} board`}>
            ...
            {board.map((row, rowIndex) => (<tr key={rowIndex}>
                {row.map((cell, colIndex) => (<SudokuCell
